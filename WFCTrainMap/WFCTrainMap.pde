...
void setup() {
  ...
  tileSize = width/ (float)SIZE - (1/(float)SIZE); // for fixing the stroke
  tiles = getTiles();
...
Tile[] getTiles() {
  ...
  //addTile(tiles, "rail_end", "GGG,GGG,GGG,GRG");
  ...
  addTile(tiles, "rail_road", "GCG,GRG,GCG,GRG");
  addTile(tiles, "rail_tunnel", "GGG,GTG,GGG,GRG");
  
  addTile(tiles, "road_straight", "GGG,GCG,GGG,GCG");
  addTile(tiles, "road_turn", "GGG,GCG,GCG,GGG");
  addTile(tiles, "road_tcross", "GGG,GCG,GCG,GCG");
  //addTile(tiles, "road_cross", "GCG,GCG,GCG,GCG");
  addTile(tiles, "road_tunnel", "GCG,GTG,GCG,GTG");
  
  addTile(tiles, "tunnel_straight", "GGG,GTG,GGG,GTG");
  //addTile(tiles, "tunnel_road", "GCG,GTG,GCG,GTG");
  
  addTile(tiles, "train_loco", "GGG,RCR,GGG,GRG");
  addTile(tiles, "train_coal", "GGG,RCR,GGG,RCR");
  addTile(tiles, "train_coal_end", "GGG,GRG,GGG,RCR");
  addTile(tiles, "train_passenger_start", "GGG,RPR,GGG,RCR");
  addTile(tiles, "train_passenger", "GGG,RPR,GGG,RPR");
  addTile(tiles, "train_passenger_end", "GGG,GRG,GGG,RPR");
...
void addTile(ArrayList<Tile> tiles, String imgPath, String edges) {
  Tile tile = new Tile(loadImage(imgPath+".png"), edges);
  tiles.add(tile);
  for (int i=1; i<4; i++)
...
void draw() {
  ...
  for (int i=0; i<SIZE; i++) {
    for (int j=0; j<SIZE; j++) {
      ...
      if (cell.collapsed) {
        noTint();
        image(tiles[cell.options[0]].img, i*tileSize, j*tileSize, tileSize, tileSize);
      } else {
        tint(255, 255/(float)cell.options.length);
        for (int o=0; o<cell.options.length; o++)
          image(tiles[cell.options[o]].img, i*tileSize, j*tileSize, tileSize, tileSize);
      }
    }
  }
}
